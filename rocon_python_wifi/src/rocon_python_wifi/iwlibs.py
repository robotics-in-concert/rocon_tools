...
def getConfiguredWNICnames():
    ...
    result = iwstruct._fcntl(wifi_flags.SIOCGIFCONF, datastr)
...
class Wireless(object):
    ...
    def setAPaddr(self, addr):
        ...
        if (addr == "AUTO" or addr == "ANY"):
            mac_addr = "\xFF"*wifi_flags.ETH_ALEN
        elif addr == "OFF":
            mac_addr = '\x00'*wifi_flags.ETH_ALEN
        else:
            if ":" not in addr:
    ...
    def getEncryption(self):
        ...
        iwpoint = Iwpoint('\x00'*wifi_flags.IW_ENCODING_TOKEN_MAX)
        status, result = self.iwstruct.iw_get_ext(self.ifname,
                                             wifi_flags.SIOCGIWENCODE,
                                             data=iwpoint.packed_data)
        iwpoint.update(result)

        if iwpoint.flags & wifi_flags.IW_ENCODE_NOKEY > 0:
            return '**'*iwpoint.length
        elif iwpoint.flags & wifi_flags.IW_ENCODE_OPEN > 0:
            return 'open'
        elif iwpoint.flags & wifi_flags.IW_ENCODE_RESTRICTED > 0:
            return 'restricted'
        elif iwpoint.flags & wifi_flags.IW_ENCODE_DISABLED > 0:
            return 'off'
    ...
    def setEncryption(self, mode):
        ...
        numeric_mode = numeric_mode & ~wifi_flags.IW_ENCODE_OPEN \
                        & ~wifi_flags.IW_ENCODE_RESTRICTED \
                        & ~wifi_flags.IW_ENCODE_DISABLED
        if (mode == 'OPEN') or (mode == wifi_flags.IW_ENCODE_OPEN):
            numeric_mode = numeric_mode | wifi_flags.IW_ENCODE_OPEN
        elif (mode == 'RESTRICTED') or (mode == wifi_flags.IW_ENCODE_RESTRICTED):
            numeric_mode = numeric_mode | wifi_flags.IW_ENCODE_RESTRICTED
        elif (mode == 'OFF') or (mode == wifi_flags.IW_ENCODE_DISABLED):
            numeric_mode = numeric_mode | wifi_flags.IW_ENCODE_DISABLED
        iwpoint = Iwpoint('\x00'*wifi_flags.IW_ENCODING_TOKEN_MAX, numeric_mode)
        status, result = self.iwstruct.iw_get_ext(self.ifname,
                                             wifi_flags.SIOCSIWENCODE,
                                             data=iwpoint.packed_data)
    ...
    def setKey(self, key, index=1):
        ...
        """
        if index not in range(1, wifi_flags.IW_ENCODE_INDEX):
            raise IndexError
        ...
        iwpoint = Iwpoint(cooked_key,
                    index + wifi_flags.IW_ENCODE_ENABLED)
        status, result = self.iwstruct.iw_get_ext(self.ifname,
                                             wifi_flags.SIOCSIWENCODE,
                                             data=iwpoint.packed_data)
    ...
    def setFrequency(self, freq):
        ...
        iwstruct = Iwstruct()
        if freq == "auto":
            iwreq = iwstruct.pack(format, -1, 0, 0, wifi_flags.IW_FREQ_AUTO)
        else:
            if freq == "fixed":
            ...
            if e > 8:
                ...
                m = int(freq_num)
                e = 0
            iwreq = iwstruct.pack(format, m, e, 0, wifi_flags.IW_FREQ_FIXED)
        status, result = iwstruct.iw_set_ext(self.ifname,
                                               wifi_flags.SIOCSIWFREQ,
                                               iwreq)
    ...
    def getMode(self):
        ...
        """
        return wifi_flags.modes[self.wireless_info.getMode()]

    def setMode(self, mode):
        """ Sets the operation mode.

        """
        this_modes = [x.lower() for x in wifi_flags.modes]
        mode = mode.lower()
        try:
...
class WirelessConfig(object):
    ...
    def getKey(self, key=0):
        ...
        # use an IW_ENCODING_TOKEN_MAX-cell array of NULLs
        #   as space for ioctl to write encryption info
        iwpoint = Iwpoint('\x00'*wifi_flags.IW_ENCODING_TOKEN_MAX, key)
        status, result = self.iwstruct.iw_get_ext(self.ifname,
                                             wifi_flags.SIOCGIWENCODE,
                                             data=iwpoint.packed_data)
        iwpoint.update(result)
    ...
    def getEssid(self):
        ...
        iwpoint = Iwpoint('\x00'*wifi_flags.IW_ESSID_MAX_SIZE)
        status, result = self.iwstruct.iw_get_ext(self.ifname,
                                             wifi_flags.SIOCGIWESSID,
                                             data=iwpoint.packed_data)
...
class WirelessInfo(WirelessConfig):
    ...
    def getSensitivity(self):
        ...
        """
        return Iwparam(self.ifname, wifi_flags.SIOCGIWSENS)
    ...
    def getBitrate(self):
        ...
        """
        return Iwparam(self.ifname, wifi_flags.SIOCGIWRATE)
    ...
    def getRTS(self):
        ...
        """
        return Iwparam(self.ifname, wifi_flags.SIOCGIWRTS)
    ...
    def getFragmentation(self):
        ...
        """
        return Iwparam(self.ifname, wifi_flags.SIOCGIWFRAG)
    ...
    def getPower(self):
        ...
        """
        return Iwparam(self.ifname, wifi_flags.SIOCGIWPOWER)
    ...
    def getTXPower(self):
        ...
        """
        return Iwparam(self.ifname, wifi_flags.SIOCGIWTXPOW)
    ...
    def getRetry(self):
        ...
        """
        return Iwparam(self.ifname, wifi_flags.SIOCGIWRETRY)
...
class Iwstruct(object):
    ...
    def iw_get_ext(self, ifname, request, data=None):
        ...
        buff = wifi_flags.IFNAMSIZE-len(ifname)
        ...
        return (result, ifreq[wifi_flags.IFNAMSIZE:])
...
class Iwrange(object):
    ...
    def __init__(self, ifname):
        self.fmt = "IIIHB6Ii4B4BB" + wifi_flags.IW_MAX_BITRATES*"i" + \
                   "2i2i2i2i3H" + wifi_flags.IW_MAX_ENCODING_SIZES*"H" + \
                   "2BBHB" + wifi_flags.IW_MAX_TXPOWER*"i" + \
                   "2B3H2i2iHB" + wifi_flags.IW_MAX_FREQUENCIES*"ihBB" + \
                   "IiiHiI"
...
class Iwscan(object):
    ...
    def getScan(self):
        ...
        """
        iwstruct = Iwstruct()
        bufflen = wifi_flags.IW_SCAN_MAX_DATA
...
class Iwscanresult(object):
    ...
    def addEvent(self, cmd, data):
        ...
        """
        if ((cmd in range(wifi_flags.SIOCIWFIRST,
                          wifi_flags.SIOCIWLAST+1)) or
            (cmd in range(wifi_flags.IWEVFIRST,
                          wifi_flags.IWEVLAST+1))):
            if cmd == wifi_flags.SIOCGIWNWID:
                pass
            elif cmd == wifi_flags.SIOCGIWFREQ:
                self.frequency = Iwfreq(data)
            elif cmd == wifi_flags.SIOCGIWMODE:
                raw_mode = struct.unpack('I', data[:4])[0]
                self.mode = wifi_flags.modes[raw_mode]
            elif cmd == wifi_flags.SIOCGIWNAME:
                self.protocol = data[:len(data)-2]
            elif cmd == wifi_flags.SIOCGIWESSID:
                self.essid = data[4:]
            elif cmd == wifi_flags.SIOCGIWENCODE:
                ...
                if (self.encode.caddr_t is None):
                    self.encode.flags = \
                        self.encode.flags | wifi_flags.IW_ENCODE_NOKEY
            elif cmd == wifi_flags.SIOCGIWRATE:
            ...
            elif cmd == wifi_flags.SIOCGIWMODUL:
                pass
            elif cmd == wifi_flags.IWEVQUAL:
                self.quality.parse(data)
            elif cmd == wifi_flags.IWEVGENIE:
                pass
            elif cmd == wifi_flags.IWEVCUSTOM:
